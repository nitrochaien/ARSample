...
open class ARTrackingManager: NSObject, CLLocationManagerDelegate {
    ...
    //===== variables
    fileprivate(set) var locationManager: CLLocationManager = CLLocationManager()
    fileprivate(set) var tracking = false
    fileprivate(set) var userLocation: CLLocation?
    fileprivate(set) var heading: Double = 0
    weak var delegate: ARTrackingManagerDelegate?
    var orientation: CLDeviceOrientation = CLDeviceOrientation.portrait {
        didSet {
     ...
     */
    func startTracking(notifyLocationFailure: Bool = false) {
    ...
    open func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        if locations.count > 0 {
            ...
            if self.reportLocationDate == nil {
                ...
                self.reportLocationTimer = Timer.scheduledTimer(timeInterval: 5, target: self, selector: #selector(reportLocationToDelegate), userInfo: nil, repeats: false)
            }
        }
    }
    
    @objc private func reportLocationToDelegate() {
    ...
    func azimuthFromUserToLocation(_ location: CLLocation) -> Double {
        ...
        azimuth = ARConfiguration.sharedInstance.radiansToDegrees(atan2(longitudeDistance, (latitudeDistance * Double(LAT_LON_FACTOR))))
        azimuth += 180.0
        
        return azimuth;
    }
    
    func startDebugMode(_ location: CLLocation) {
        self.debugLocation = location
        self.userLocation = location;
    }
    
    func stopDebugMode(_ location: CLLocation) {
    ...
    func startLocationSearchTimer(resetStartTime: Bool = true) {
        ...
        self.locationSearchTimer = Timer.scheduledTimer(timeInterval: 5, target: self, selector: #selector(locationSearchTimerTick), userInfo: nil, repeats: false)
